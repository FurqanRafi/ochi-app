...
const Header = () => {
    ...
    return (
        <header className="fixed top-0 left-0 w-full py-4 md:py-5 backdrop-blur-md bg-[#f2f2f2]/70 shadow-md z-50">
            <div className="flex Mycontainer1 items-center justify-between">
                ...
                {/* Desktop Nav */}
                <nav className="hidden sm:flex flex-wrap justify-center gap-6 md:gap-10 text-sm md:text-base">
                    {navItems.map((item, index) => (
                        <Link key={index} href={`/${item.replace(/\s+/g, '').toLowerCase()}`}>
                            <div className={`capitalize text-gray-800 transition duration-200 font-light ${index === 4 ? 'ml-0 md:ml-20' : ''} cursor-pointer group overflow-hidden`}>
                                {item.split('').map((letter, i) => (
                                    <span key={i} className="inline-block animation-letter" style={{ animationDelay: `${i * 0.05}s` }}>
                                        {letter}
                                    </span>
                                ))}
                            </div>
                ...
                {/* Mobile Menu Icon */}
                <button className="sm:hidden" onClick={toggleMenu} aria-label="Open Menu">
            ...
            {/* Mobile Menu Overlay */}
            <div
                className={`fixed top-0 left-0 w-full h-screen z-[999] bg-black text-white transition-all duration-500 ease-in-out transform ${menuOpen ? "translate-y-0 opacity-100 visible" : "-translate-y-full opacity-0 invisible"
                    } flex flex-col items-center justify-center`}
            >
                {/* Close Button */}
                <button className="absolute top-5 right-5" onClick={closeMenu} aria-label="Close Menu">
                ...
                {/* Menu Links */}
                <nav className="flex flex-col gap-15 text-3xl text-center">
                    {navItems.map((item, index) => (
                        <Link key={index} href={`/${item.replace(/\s+/g, '').toLowerCase()}`}>
                            <div className="capitalize text-white hover:text-blue-400 font-light cursor-pointer" onClick={closeMenu}>
                                {item}
                            </div>
                        </Link>
                    ))}
