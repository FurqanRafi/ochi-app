...
const Header = () => {
  ...
  {navItems.map((item, index) => (
    <Link key={index} href={`/${item.replace(/\s+/g, '').toLowerCase()}`}>
      <div
        className={`capitalize text-gray-800 transition duration-200 font-light ${index === 4 ? 'ml-0 md:ml-20' : ''} cursor-pointer group overflow-hidden`}
      >
        {item.split('').map((letter, i) => (
          ...
          >
            ...
            {menuOpen && (
                <div className="fixed inset-0 bg-white z-[998] flex flex-col p-6">
                    ...
                    <nav className="flex flex-col gap-6 text-base text-left">
                        {navItems.map((item, index) => (
                            <Link key={index} href={`/${item.replace(/\s+/g, '').toLowerCase()}`}>
                                <div
                                    className="capitalize text-gray-800 hover:text-blue-600 font-light cursor-pointer"
                                    onClick={closeMenu}
                                >
                                    {item}
                                </div>
